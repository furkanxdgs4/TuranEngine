#include "TuranAPI/TSTL/Bitset.h"
#include <iostream>

namespace TuranAPI {
	namespace STL {
		Bitset::Bitset(size_t byte_length, TuranAPI::MemoryManagement::IAllocator* Allocator_) : buffer_length(byte_length), Allocator(Allocator_) {
			boolarray = (bool*)Allocator->Allocate_MemoryBlock(sizeof(bool) * byte_length);
		}
		...
		void Bitset::Expand(size_t expand_size) {
			bool* new_block = (bool*)Allocator->Allocate_MemoryBlock(expand_size + buffer_length);
			if (new_block) {
				...
				Allocator->Delete_MemoryBlock(boolarray);
				boolarray = new_block;
			}
			else {
