...
Material_Import_Window::Material_Import_Window()
	: IMGUI_WINDOW("Material Type Import"), Material_Uniforms(LASTUSEDALLOCATOR, 4, 4), selectlist_vector(LASTUSEDALLOCATOR, 4, 10), UNIFORM_VAR_TYPE_NAMEs(LASTUSEDALLOCATOR, 0)
{
...
void Material_Import_Window::Run_Window() {
	...
	if (IMGUI->Button("Import")) {
		...
		for (size_t i = 0; i < TuranEditor::EDITOR_FILESYSTEM->Get_AssetList().size(); i++) {
			TuranAPI::Resource_Type* RESOURCE = TuranEditor::EDITOR_FILESYSTEM->Get_AssetList()[i];
		...
		for (unsigned int i = 0; i < Material_Uniforms.size(); i++) {
			GFX_API::Material_Uniform& UNIFORM = Material_Uniforms[i];
		...
		TuranAPI::Resource_Type* imported_resource = Material_Type_Loader::Load_MaterialType(MATERIALTYPE_VERTEX_PATH, MATERIALTYPE_FRAGMENT_PATH, PATH, &Material_Uniforms, &status);
		TuranEditor::Status_Window* error_window = new TuranEditor::Status_Window(status);
	...
	if (IMGUI->Button("Read Material Type")) {
		VERTEX_SOURCE = *TuranAPI::FileSystem::Read_TextFile(MATERIALTYPE_VERTEX_PATH, LASTUSEDALLOCATOR);
		FRAGMENT_SOURCE = *TuranAPI::FileSystem::Read_TextFile(MATERIALTYPE_FRAGMENT_PATH, LASTUSEDALLOCATOR);
...
TuranAPI::Resource_Type* Material_Type_Loader::Load_MaterialType(const char* vertex_path, const char* fragment_path, const char* output_path, Vector<GFX_API::Material_Uniform>* material_inputs, String* compilation_status) {
	...
	ShaderSource* vertex_shader = new ShaderSource;
	...
	ShaderSource* fragment_shader = new ShaderSource;
	...
	for (unsigned int i = 0; i < material_inputs->size(); i++) {
		GFX_API::Material_Uniform& material_uniform = (*material_inputs)[i];
