...
static GFX_API::Material_Instance* Create_Instance_ofMaterialType(GFX_API::Material_Type_Resource* material_type, Vector<GFX_API::Material_Uniform>& Uniform_List, const char* output_path);
...
void MaterialInstance_CreationWindow::Run_Window() {
	...
	Vector<GFX_API::Material_Uniform> UNIFORM_LIST(LASTUSEDALLOCATOR, 2, 4);

	if (IMGUI->Button("Create")) {
		GFX_API::Material_Instance* created_matinst = Create_Instance_ofMaterialType(selected_materialtype, UNIFORM_LIST, PATH);
	...
	for (unsigned int i = 0; i < TuranEditor::CONTENTMANAGER->Read_ALLMaterialTypes()->size(); i++) {
		Resource_Type* RESOURCE = TuranEditor::CONTENTMANAGER->Read_ALLMaterialTypes()->Get(i);
	...
	if (IMGUI->SelectList_OneLine(nameof_selectlistline, &selected_listitemindex, &item_names)) {
		selected_materialtype = TuranEditor::CONTENTMANAGER->Read_ALLMaterialTypes()->Get(selected_list_item_index);
		for (unsigned int i = 0; i < selected_materialtype->UNIFORMs.size(); i++) {
			GFX_API::Material_Uniform& UNIFORM = selected_materialtype->UNIFORMs[i];
	...
	if (IMGUI->Begin_Tree("Uniform List")) {
		for (unsigned int i = 0; i < UNIFORM_LIST.size(); i++) {
			GFX_API::Material_Uniform* UNIFORM = &UNIFORM_LIST[i];
...
GFX_API::Material_Instance* Create_Instance_ofMaterialType(GFX_API::Material_Type_Resource* material_type, Vector<GFX_API::Material_Uniform>& UNIFORM_LIST, const char* output_path) {
