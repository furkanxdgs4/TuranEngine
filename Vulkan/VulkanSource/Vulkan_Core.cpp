...
namespace Vulkan {
	...
	void Vulkan_Core::Initialization() {
		...
		GFX_API::Material_Type_Resource FirstTriangle_MatType;
		FirstTriangle_MatType.NAME = "FirstTriangle_MatType";
		FirstTriangle_MatType.LANGUAGE = TuranAPI_ENUMs::VULKAN;
	...
	void Vulkan_Core::New_Frame() {
		...
		vkAcquireNextImageKHR(VK_GPU->Logical_Device, VK_WINDOW->Window_SwapChain, UINT64_MAX, Wait_GettingFramebuffer, VK_NULL_HANDLE, &SwapchainImage_Index);
		std::cout << "Current SwapchainImage_Index: " << SwapchainImage_Index << std::endl;
		vkWaitForFences(VK_GPU->Logical_Device, 1, &SwapchainFences[SwapchainImage_Index], true, UINT64_MAX);
		vkResetFences(VK_GPU->Logical_Device, 1, &SwapchainFences[SwapchainImage_Index]);
		VkSubmitInfo CB_si = {};
		CB_si.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
		CB_si.pNext = nullptr;
		VkSemaphore WaitSemaphores[] = { Wait_GettingFramebuffer };
		VkPipelineStageFlags WaitStages[] = { VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT };
		CB_si.waitSemaphoreCount = 1;
		CB_si.pWaitSemaphores = WaitSemaphores;
		CB_si.pWaitDstStageMask = WaitStages;

		CB_si.commandBufferCount = 1;
		CB_si.pCommandBuffers = &FirstTriangle_CommandBuffers[SwapchainImage_Index];

		VkSemaphore SignalSemaphores[] = { Wait_RenderingFirstTriangle };
		CB_si.signalSemaphoreCount = 1;
		CB_si.pSignalSemaphores = SignalSemaphores;
		if (vkQueueSubmit(VK_GPU->Queues[0], 1, &CB_si, SwapchainFences[SwapchainImage_Index]) != VK_SUCCESS) {
			TuranAPI::LOG_CRASHING("Command Buffer submit has failed!");
		}
		...
		SwapchainImage_PresentationInfo.pWaitSemaphores = SignalSemaphores;
		
		VkSwapchainKHR Swapchains[] = { VK_WINDOW->Window_SwapChain };
		SwapchainImage_PresentationInfo.swapchainCount = 1;
		SwapchainImage_PresentationInfo.pSwapchains = Swapchains;
		SwapchainImage_PresentationInfo.pImageIndices = &SwapchainImage_Index;
		SwapchainImage_PresentationInfo.pResults = nullptr;
		if (vkQueuePresentKHR(VK_GPU->Queues[0], &SwapchainImage_PresentationInfo) != VK_SUCCESS) {
			TuranAPI::LOG_CRASHING("Submitting Presentation Queue has failed!");
		}
	}
